...
elseif($_POST["mode"]=="send"){comp_form();}
...
for($i = 0; $i <= 7; $i++){
   if (strstr($connectstr, $com[$i])) {
      $chk = '●'.$com[$i]."<br>";
      $connectstr=str_replace($com[$i],$chk,$connectstr);
      }
};

  $data=str_replace("!checkbox!",$connectstr,$data);
...
function error($msg){
  $error=fopen("error.html","r");
  $size=filesize("error.html");
  ...
  $data=str_replace("!add!",$add,$data);
  $data=str_replace("!tel!",$tel,$data);
  $data=str_replace("!mail!",$mail,$data);
  $data=str_replace("!mail2!",$mail2,$data);
...
#本文
  ...
  $checkbox=str_replace("<br>"," ",$checkbox);
